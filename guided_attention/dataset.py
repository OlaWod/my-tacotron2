...
class MyDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        W_mat = self.get_W_mat((len(batch), max_target_len, max_input_len),
                               output_lengths,
                               input_lengths)

        return text_padded, input_lengths, mel_padded, gate_padded, output_lengths, W_mat


    def get_W_mat(self, padded_size, mel_lens, text_lens, g=0.2):
        '''
         padded_size = (batch, padded_mel_len, padded_text_len)
         mel_lens: (batch)
         text_lens: (batch)
         W_mat : (batch, padded_mel_len, padded_text_len)
        '''
        batch = padded_size[0]
        W_mat = torch.zeros(padded_size)

        for i in range(batch):
            N = mel_lens[i]
            T = text_lens[i]
            ns = torch.arange(N).unsqueeze(-1).expand(N, T)
            ts = torch.arange(T).unsqueeze(0).expand(N, T)
            W_mat[i, :N, :T] = torch.FloatTensor(1 - torch.exp(- (ns/N - ts/T)**2 / (2*g**2)))
