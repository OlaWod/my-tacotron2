...
class MyLoss(nn.Module):
    ...
    def forward(self, pred, truth):
        mel_pred, mel_pred_postnet, stop_pred, alignment = pred
        ...
        W_mat = truth[-1]
        attention_loss = 1 * torch.mean(W_mat * alignment)

        total_loss = mel_loss + mel_postnet_loss + stop_loss + attention_loss

        return total_loss, mel_loss, mel_postnet_loss, stop_loss, attention_loss
